...
def format_plugin_package_status(report: dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        "- Install OSRSFlipper Telemetry through RuneLite Plugin Hub for normal RuneLite.",
        "- The dev client remains a local troubleshooting fallback.",
        "",
        "Checks:",
    ]
    ...
    checklist.write_text(
        "\n".join(
            [
                ...
                "7. Plugin Hub approval is complete; install OSRSFlipper Telemetry from RuneLite's Plugin Hub in the Jagex-launched client.",
                "8. Keep the dev client only as a local troubleshooting fallback.",
                "",
                "Fallback command: `python runelite_telemetry_control.py start-dev`.",
                "",
            ]
        ),
        encoding="utf-8",
